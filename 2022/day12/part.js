...
export class Part extends Day {
    ...
    async part1(data) {
        this.map = data.map((d,i) => {
            return d.split('').map((r,j) => ({
                ...
                y: i,
                elevation: r,
                value: !!r.match(/S/) ? -1 : !!r.match(/E/) ? 26 : r.charCodeAt(0) - 97,
                name: `${j}-${i}-${r}`,
                visited: false,
                isPath: false
            }));
    ...
    async breadthFirst(graph, start, end) {
        ...
        while (queue.length > 0) {
            ...
            console.log('last path', path.slice())
    ...
    createPath(list) {
        console.log('creat path', list.slice());
        ...
        return new Promise((res,rej) => {
            setTimeout(() => {
                let newRects = this.svg.selectAll('.tiles').data(data)
                    ...
                    .merge(this.rects)
                        .attr('stroke', d => d.visited ? (d.isPath ? 'steelblue' : 'black') : 'transparent')
